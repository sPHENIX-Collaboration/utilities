						   		...
						   		extensions: [               
							     	...
							     	[
							   			$class: 'PreBuildMerge',
							    			options: [
											mergeRemote: 'origin',
							  				mergeTarget: 'QA-tracking-distortions'
							  			]
							    	],   
							     	[
							   			$class: 'PreBuildMerge',
			...
			steps 
			{
				timestamps { 
					ansiColor('xterm') {
						...
						dir('reference')
						{
    					copyArtifacts(projectName: "test-tracking-distortions-qa-reference", selector: lastSuccessful());
	...
	post {
		...
		always{
			...
			{								
				...
				def report_content = "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA with Distortions: [build is ${currentBuild.currentResult}](${env.BUILD_URL}), [:bar_chart: trends](${env.JOB_URL}/plot/)";	        
				...
				writeFile file: "report/QA-tracking-distortions-qa.md", text: "${report_content}"	
			...
			build(job: 'github-commit-checkrun',
				...
				[
					...
					string(name: 'output_summary', value: "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA with Distortions: [build is ${currentBuild.currentResult}](${env.BUILD_URL})"),
		...
		success {
			script {
				currentBuild.description = "${currentBuild.description}<br><button onclick=\"window.location.href='${JENKINS_URL}/job/sPHENIX/job/test-tracking-distortions-qa-reference/parambuild/?ref_build_id=${BUILD_ID}';\">Use as QA reference</button>" 
			}
