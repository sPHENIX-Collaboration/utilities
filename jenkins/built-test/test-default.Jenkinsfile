	...
	stages { 
		...
		stage('Test')
		{
			...
			steps 
			{
				...
				sh("/usr/bin/singularity exec -B /cvmfs -B /gpfs -B /direct -B /afs -B /sphenix /cvmfs/sphenix.sdcc.bnl.gov/singularity/rhic_sl7_ext.simg  utilities/jenkins/built-test/test-default.sh ${macro_name} 30 0")
