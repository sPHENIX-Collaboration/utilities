					...
					ansiColor('xterm') {
                        ...
                        dir('report')
                        {
                            ...
                            script
                            {
                                ...
                                def report_content = """
## For repository maintainers, please start the CI check manually ([feedback](mailto:jhuang@bnl.gov))
...
"""
		...
		stage('clang-tidy')
		{
			steps 
			{
				script
				{
				def built = build(job: 'clang-tidy-macros-pipeline',
					parameters:
					[
					string(name: 'checkrun_repo_commit', value: "${checkrun_repo_commit}"), 
					string(name: 'sha_macros', value: "${sha1}"), 
					string(name: 'git_url_macros', value: "https://github.com/${ghprbGhRepository}.git"), 
