...
//    }
				...
				timestamps {
					ansiColor('xterm') {
		    			...
		    			[
			    			...
			    			string(name: 'LabelCategory', value: "track-high-occ-QA"),
						   		...
						   		extensions: [               
							     	...
							     	[
							   			$class: 'PreBuildMerge',
							    			options: [
											mergeRemote: 'origin',
							  				mergeTarget: 'QA-tracking-high-occupancy'
							  			]
							    	],   
							     	[
							   			$class: 'PreBuildMerge',
			...
			steps 
			{
				timestamps { 
					ansiColor('xterm') {
						...
						dir('reference')
						{
    					copyArtifacts(projectName: "test-tracking-high-occupancy-qa-reference", selector: lastSuccessful());
	...
	post {
		...
		always{
			...
			//  writeFile file: "QA-tracking-high-occupancy-qa.md", text: "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA at high occupancy: [build is ${currentBuild.currentResult}](${env.BUILD_URL})"				
			dir('report')
			{
			...
			{								
				...
				def report_content = "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA at high occupancy: [build is ${currentBuild.currentResult}](${env.BUILD_URL})";	        
			...
			build(job: 'github-commit-checkrun',
				...
				[
					...
					string(name: 'output_summary', value: "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA at high occupancy: [build is ${currentBuild.currentResult}](${env.BUILD_URL})"),
		...
		success {
			script {
				currentBuild.description = "${currentBuild.description}<br><button onclick=\"window.location.href='${JENKINS_URL}/job/sPHENIX/job/test-tracking-high-occupancy-qa-reference/parambuild/?ref_build_id=${BUILD_ID}';\">Use as QA reference</button>" 
			}
