...
//    }
	...
	stages { 
								...
								[
						   		...
						   		extensions: [               
							     	...
							     	[
							    		...
							    		options: [
											mergeRemote: 'origin',
							  			mergeTarget: 'tracking-high-occupancy-qa'
							  			]
							    	],   
							     	[
							   			$class: 'PreBuildMerge',
		...
		stage('Test')
		{
			...
			steps 
			{
				...
				sh('$singularity_exec_sphenix tcsh -f utilities/jenkins/built-test/test-tracking-high-occupancy-qa.sh $build_type $num_event $number_jobs')
	...
	post {
		...
		always{
			...
			dir('report')
			{
			  writeFile file: "QA-tracking-high-occupancy-qa.md", text: "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA at high occupancy: [build is ${currentBuild.currentResult}](${env.BUILD_URL}), [:bar_chart:QA report - Tracking/high occ.](${env.BUILD_URL}/QA_20Report/) "				
			}
			...
			build(job: 'github-commit-checkrun',
				...
				[
					...
					string(name: 'output_summary', value: "* [![Build Status ](${env.JENKINS_URL}/buildStatus/icon?job=${env.JOB_NAME}&build=${env.BUILD_NUMBER})](${env.BUILD_URL}) Tracking QA at high occupancy: [build is ${currentBuild.currentResult}](${env.BUILD_URL}), [:bar_chart:QA report - Tracking/high occ.](${env.BUILD_URL}/QA_20Report/) "),
