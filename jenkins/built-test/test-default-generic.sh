...
if (( $run_valgrind > 0 )); then
	...
	valgrind_prefix="valgrind -v --num-callers=30 --max-threads=1500 --gen-suppressions=all --leak-check=full --error-limit=no --log-file=${macro_file_name}.valgrind $valgrind_sup --xml=yes --xml-file=${macro_file_name}.valgrind.xml --leak-resolution=high"
