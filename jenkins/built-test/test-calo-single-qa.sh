...
do
	...
	echo "======================================================="
  	...
  	( /usr/bin/time -v  timeout --preserve-status --kill-after=1s --signal=9 1d  root.exe -b -q "Fun4All_G4_sPHENIX.C(35,"\"${particle_ID}\"",${pT_GeV},"\"G4sPHENIX_${job_name}\"")" && echo $? > exit_code_${id_number}.log ) 2>&1 | tee G4sPHENIX_${job_name}.log | (head; tail)  &
...
do 
	...
	summary="* [:bar_chart: ${filename} for ${particle_ID} at p_T=${pT_GeV}GeV](https://nbviewer.sphenix.bnl.gov/github/sPHENIX-Collaboration/QA-gallery/blob/${git_tag}/${nbname})"
