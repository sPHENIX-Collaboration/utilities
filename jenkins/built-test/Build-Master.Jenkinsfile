...
//    }
	...
	post {
		always{
				...
				if ("$build_type" == 'clang') {
					recordIssues enabledForFailure: true, failedNewHigh: 100, failedNewNormal: 100, tool: clang(pattern: 'build/${build_type}/rebuild.log')
				} else if ("$build_type" == 'scan') {
					recordIssues enabledForFailure: true, failedNewHigh: 100, failedNewNormal: 100, tool: clangAnalyzer(pattern: 'build/${build_type}/scanlog/*/*.plist')
				} else {
