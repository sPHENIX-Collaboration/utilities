	...
	post {
		always{
				...
				if ("$build_type" == 'clang') {
					...
					recordIssues enabledForFailure: true, failedNewHigh: 1, failedNewNormal: 10, tool: clangAnalyzer(pattern: 'build/${build_type}/scanlog/*/*.plist')
				} else {
					recordIssues enabledForFailure: true, failedNewHigh: 1, failedNewNormal: 10, tool: gcc(pattern: 'build/${build_type}/rebuild.log')
				}
        		} // script 
