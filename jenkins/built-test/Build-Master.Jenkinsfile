			...
			//		stages{
						...
						stage('sPHENIX-Build')
						{
							...
							steps 
							{
								...
								sh('$singularity_exec_sphenix_farm  sh utilities/jenkins/built-test/full-build.sh')
	...
	post {
		always{
				...
				if ("$build_type" == 'clang') {
					...
					recordIssues enabledForFailure: true, failedNewHigh: 5, failedNewNormal: 5, tool: clangAnalyzer(pattern: 'build/${build_type}/scanlog/*/*.plist')
				} else {
