			...
			stage('Build-Test') {
									...
									steps 
									{
						    			...
						    			[
							    			...
							    			string(name: 'git_url_coresoftware', value: "${ghprbAuthorRepoGitUrl}"), 
...
[![sPHENIX](https://raw.githubusercontent.com/sPHENIX-Collaboration/utilities/master/jenkins/material/sphenix-logo-white-bg-72p.png)](https://www.sphenix.bnl.gov/web/) &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; [![jenkins.io](https://raw.githubusercontent.com/sPHENIX-Collaboration/utilities/master/jenkins/material/jenkins_logo_title-72p.png)](https://jenkins.io/)"""
		...
		success {
			slackSend (color: '#00FF00', message: "Pull request ${ghprbPullId} check SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})  for ${ghprbPullLink}")
		}
		failure {
			slackSend (color: '#FF0000', message: "Pull request ${ghprbPullId} check FAILED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})  for ${ghprbPullLink}")
		}
		unstable {
			slackSend (color: '#FFF000', message: "Pull request ${ghprbPullId} check UNSTABLE: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})  for ${ghprbPullLink}")
		}
	}
